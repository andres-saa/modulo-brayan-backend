<template>
    <div>
      ...
      <Button @click="showProductDialog = !showProductDialog" style="background-color: black; border-radius: 0.5rem; font-weight: bold;margin-bottom: 2rem;">AGREGAR CATEGORIA</Button>
      <div class="grid ">
        <div class="col-12 md:col-6 lg:col-4 xl:col-3" v-for="product in categories" :key="product.id">
...
<div class="botones col-12" style="display: flex; justify-content: end;">
  ...
  <button @click="deleteProduct(product.category_id)" style="color:white; background-color: rgb(255, 83, 83); border-radius: 0.5rem;border: none; font-weight: bold; padding:0.5rem 1rem ;  right: 1rem;">Eliminar</button>
    ...
    <!-- <input type="file" @change="handleFileChange" /> -->
                    ...
                    <div class="dialog-container">
                      ...
                      <div class="field">
                        ...
                        <Button class="text-center" @click="createCategory" style="background-color: black; border-radius: 0.5rem; font-weight: bold;margin-bottom: 2rem;"><span class="text-center col-12 p-0">AGREGAR PRODUCTO</span></Button>
...
import { showSiteDialog } from '../../service/state';
  ...
  try {
    // Intenta obtener las categorías del localStorage
    const storedCategories = localStorage.getItem('categories');

    // Si hay categorías en el localStorage, asigna directamente a categories.value
    if (storedCategories) {
      categories.value = JSON.parse(storedCategories);
    } else {
      // Si no hay categorías en el localStorage, realiza la llamada a la API
      ...
      // Asigna las categorías a la variable de estado
      categories.value = data;

      // Guarda las categorías en el localStorage
...
const deleteProduct = async (productId) => {
  ...
  try {
    ...
    const response = await fetch(`${URI}/categories/${productId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
